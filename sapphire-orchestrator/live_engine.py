...
def _run_one_bucket1_agent(
    ...
    * No reference to ``all_dossier_facts``, ``agent_statuses``, etc. — those are
    ...
    Returns
    -------
    ``(agent_id, res, corpus_cards)`` on success, or ``None`` if the agent is not
    in the registry (caller skips it).
    """
    ...
    return (agent_id, res, corpus_cards)
...
def run_live(
    ...
    *,
    ...
    # Submit all registered Bucket-1 agents concurrently. Each agent is I/O-bound
    ...
    #     collected from futures AFTER the pool joins, in _BUCKET1_AGENTS order.
    _num_scheduled = sum(1 for aid in _BUCKET1_AGENTS if aid in known_ids)
    _n_workers = min(_MAX_BUCKET1_WORKERS, max(1, _num_scheduled))

    # Map agent_id → Future result (submitted in _BUCKET1_AGENTS order so the
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=_n_workers) as _pool:
        for agent_id in _BUCKET1_AGENTS:
            ...
            fut = _pool.submit(
                ...
                batched_outputs, on_progress,
            )
            _futures[agent_id] = fut
    # Pool __exit__ blocks until all futures complete — the merge below is safe.

    # Merge results in the ORIGINAL _BUCKET1_AGENTS order so discover.agents and
    ...
    for agent_id in _BUCKET1_AGENTS:
        ...
        _, res, corpus_cards = result_tuple
