...
1. **ASO-tox isn't fed sequences in a live run (ACTIVE TASK).** The `aso-tox` tool is fully integrated and correct — called directly it returns real GBR/Hagedorn scores, and it's registered, in the Bucket-1 list, and dispatches in `run_live`. **But `run_live` has no input channel for ASO sequences**, so in a normal flow the agent runs with zero sequences and honestly returns `[]` — the orchestrator *calls* it but never *feeds* it. Fix: give `run_live` a `sequences=` path (the same channel the future ASO-Design tool will populate) and thread it into the agent inputs, with a test proving tox facts land in the dossier. Brief: `docs/superpowers/plans/2026-06-22-aso-tox-sequence-wiring.md`. (First exercise of the new dev harness.)
2. **The live path isn't wired to the front door.** `serve.py`/Console still use the canned engine (or a single Claude reconstruction) for novel queries — they don't yet call `run_live`. **The keystone product task**: route the Console/CLI through `run_live` so *any* user question runs the full harnessed firm. (Once done, the trace viewer shows it for free.)
3. **Real backends in `run_live` have never been run for real.** Personas (Claude), EMET, and Q-Models are **mocked** in verification (the "no token/$ burn" rule). Flipping `ctx` to the live Claude runner + a logged-in EMET session + the real Q-Models client is proven to *dispatch*, but no paid end-to-end run has executed.
4. **Institutional personas aren't individually dispatched.** `run_live` seats every Bucket-2 slot through the generic `company-partner` template; `ex-fda-regulator`, `adversarial-red-team`, `payer-partner`, `kol-partner` are registered but not invoked. Either wire them per-seat or formally adopt the template approach (and stop implying otherwise in tests).
5. **Q-Models depth.** CPU tracks + GPU **dry-run** only; **one real GPU eval has never been run**; some tracks are stubs.
6. **Moat scoring vs. Loka.** The real EP-distance substrate works, but Loka's higher-level rescue scoring (e.g. reproducing "rapamycin rescues TSC2") is **not** reproduced — gated on getting Loka's repo + 7-stage workflow doc (the external asks).
7. **Scenario coverage.** 6 of the ~300 target questions captured (`nav1_8, tsc2, lrrk2_pd, scn2a_epilepsy, gba1_pd, c9orf72_als`); named stubs remain in the manifest. The capture pipeline is proven (one live-EMET pass each); the system can already *run* any query via `run_live`.
8. **The loop needs real outcomes.** It runs on seeded/demo outcomes; real wet-lab `record_outcome`s are needed for the blind-spot/calibration machinery to actually improve predictions, and to justify moving governance from advisory toward autonomous.
9. **Transparency is terminal-only.** No Console/web view of the trace yet (deliberately deferred — "no fancy UI").
10. **Smaller gaps.** `recall` keys on genes, so a disease-only query that plans as "general CNS" recalls 0 priors; EMET is Playwright (an EMET **MCP** would replace the browser seam); the moat DB must be rebuilt locally (`python3 _build/build_moat_db.py`) since it's gitignored.

### Suggested next step
**#1 is the active task** (wire ASO sequences into `run_live` so the tox tool produces dossier facts end-to-end) — the first real exercise of the dev harness. Then **#2** (wire `run_live` into `serve.py`/Console with real backends) converts the proven-but-isolated live engine into the actual product, and #9 (web trace view) falls out almost for free. #7 (coverage) and #8 (real outcomes) make it broad and self-improving; #5/#6 are depth/partnership items.
