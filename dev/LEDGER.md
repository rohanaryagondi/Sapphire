...
- Gaps/Follow-ups: streaming is a P1-explicit non-goal for this phase (plain JSON POST/response, matching `/api/step-chat`) — a fast-follow if the answer latency (comparable to `report.py`'s ~70–100s full-context synthesis) warrants it. Phases 2–5 of WO-9 (harden the live run, inspectable sources, robust planner, workspace) remain queued.
