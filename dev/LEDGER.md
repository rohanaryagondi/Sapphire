...
- Gates: **scope CLEAN** (exactly `dev/validate-corpus.sh` + `dev/HELP.md`; no corpus content, no engine code) · **functional — validator still validates CLEAN** (`bash dev/validate-corpus.sh`): **post-market-safety** 8 cards schema+tier ok + URLs live (exit 0), **patent-ip** 7 cards schema+tier ok + URLs live (exit 0, mktemp path exercised end-to-end) · **new T1-patent path proven, not vacuous** — a T1 `patents.google.com` card PASSES the tier-domain check on the new script but FAILS on origin/main (`T1 non-primary 'patents.google.com'`; old script confirmed to have zero `PRIMARY_PATENT`); note the committed patent-ip corpus is still all-T2 (Hayes re-tiers the 5 granted-US cards to T1 post-merge, as the HELP answer directs) · full suite **GREEN 591** (`dev/run-tests.sh`, exit 0; contracts 52 · harness 83 · emet 26 · memory 14 · selfimprove 20 · moat 68 · corpus 8 · tests 275 · frontend 45; the documented `test_consult_round1` load-flake did NOT occur — no isolated re-run needed) · **no secrets** (zero hits of `Roharygon2006`/`sk_bc_ws_live`/`SAPPHIRE_EMET_PASS`/`AKIA`/private-key in the diff) · **no binaries** (numstat all-text) · `Built-By: rohan` + Claude `Co-Authored-By` trailers present.
...
- Gates: full suite **GREEN 591** (`dev/run-tests.sh`, boto3 1.43.27 present so `TestPresign` ran and passed — verified the 2 presign cases reported `ok`, not `skipped`; no `test_consult_round1` load-flake) · **no secrets** (zero hits of `Roharygon2006`/`sk_bc_ws_live`/`SAPPHIRE_EMET_PASS`/`AKIA`/private-key) · **no binaries** (text-only) · `Built-By: rohan` + Claude `Co-Authored-By` trailers present.
