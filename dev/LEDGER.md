...
## 2026-06-28 — WO-2 B-6: URL-param preselection for shareable ?mode=replay demo link  (`rohan/front-door-console`, SHA `6de8197`)
- Built-By: `rohan`. **Frontend-only** — 2 files changed: `frontend2/static/app.js` (+18 lines) + `frontend2/tests/test_server.py` (+20 lines). Adds a 16-line IIFE to app.js that reads `URLSearchParams` on page load: `?mode=replay` → preselects the `replay` profile; `?profile=<name>` → any valid dropdown value. Default unchanged when no param. Test anchors to the IIFE's unique literals (`params.get("mode") === "replay"`, `profileSel.value = target`).
- Gates: **721 tests GREEN** (pre-push hook re-ran them) · review **Approved with notes** (fixed: `var`→`const`, two vacuous test assertions tightened to IIFE-unique literals) · Gate 5 SSE driven: `simulate` profile streamed 70 events (open + 68 progress across all 22 Bucket-1 agents + 5 personas round1 + 5 rebuttals + synthesis + done), `result._via=harness-live`, `_simulated=True`, `consult.round2` len=5, `spread.stance_mix={'conditional':5}`, synthesis "Conditional advance". `replay` profile returned frozen capture in 0.02 s ($0), `_via=replay`, `_replay=True`. Served JS confirmed `URLSearchParams` IIFE live via curl. No engine/server/stdlib boundary change.
- Gaps/Follow-ups: none. The WO-2 B-6 item is complete. Not yet merged to main — awaiting Rohan's Claude PR review + Gate 6 (standard tier, Gate 6 n/a but PR approval required per conventions).
