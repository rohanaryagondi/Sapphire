...
## 2026-06-28 — WO-5: Moat rescue-ranking fix — faithful Loka dual-rank union method  (`rohan/moat-rescue-loka`, PR pending)
- Built-By: `rohan`. Ported Loka's dual-rank union scoring to the Sapphire SQLite moat. **The bug**: `build_moat_db.py` kept only cosine-top-K rows per partition — a euclidean-strong candidate (cosine-rank > K) was evicted before its euclidean rank was ever computed. **The fix**: two bounded heaps per `(query, effect, ref_type)` partition — one cosine-heap and one euclidean-heap; the union of both survives. Stored `rank_cosine`, `rank_euclidean`, `union_rank` (replaces single `rank`). `MoatClient.neighbors()` now orders by `union_rank ASC` (faithful to Loka's `ORDER BY rank_cosine + rank_euclidean ASC`). `moat/facts.py` adds a `rescue_genes` (opposite-effect gene) fact category and `supporting_genes` field on all facts. **Fidelity verified**: for TSC2 opposite-gene partition, 50/50 cosine-heap and 50/50 euclidean-heap entries have exact true-global ranks (hand-verified against Loka's ROW_NUMBER() OVER PARTITION SQL). Cross-heap sentinels (K+1=51) are a documented property of the heap approach (Loka's materialized view has all rows; we use bounded heaps). **Key data reality confirmed**: MTOR (rank_cosine=51 sentinel, rank_euclidean=27) is SAVED by the euclidean heap — old code would have dropped it. Rapamycin/sirolimus/everolimus not in EP-distance top-K for TSC2 compounds — accepted data outcome. 282,801 euclidean-only survivors rescued across all partitions.
- Gates: **395 tests green** (301 integration + 94 moat; 7 skipped; 0 failures) · independent review **Approved** (3 minor findings, none blocking) · provenance `moat-real` unchanged, no secrets/binaries · stdlib-only runtime, pyarrow only in `_build/main()` · **Gate 5**: DB rebuilt from real 38.4M-row parquet; TSC2 gene rescue query printed (DCTN6 union=2, FZD7 union=4, RALA union=10 …); MTOR confirmed rescued; heap-union fix stats shown (282K euclidean-only survivors); `moat_facts` returns 15 facts (5 similar-gene, 5 rescue-gene, 5 rescue-compound) with `supporting_genes=1` on all.
- Gaps/Follow-ups: canonical `RohanOnly/moat/moat.sqlite` must be rebuilt (new 11-column schema) before merging — `python _build/build_moat_db.py` (~9 min); sentinel ranks (K+1) are approximate for cross-heap rows — a two-pass scan would give exact ranks at the cost of holding all partition rows in memory; `supporting_genes > 1` semantics deferred (would require tracking per-query-ID aggregation).
