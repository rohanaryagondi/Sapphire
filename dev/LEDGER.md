...
## 2026-06-29 — Wave-1 persistence backend — conversations + run history (SQLite, stdlib)  (`rohan/persistence-backend`, SHA `897f6ae`)
- Built-By: `rohan`. New `sapphire-orchestrator/store.py` — stdlib-only SQLite3 persistence layer (3 tables: `conversations`, `messages`, `runs`; WAL mode; new connection per call for thread safety; DB at `RohanOnly/sapphire.sqlite`, overridable via `SAPPHIRE_STORE_DB` env var). 12 public functions: `create_conversation`, `list_conversations` (preview from first user message, newest-first ordering), `get_conversation`, `get_run`, `rename_conversation`, `set_starred`, `delete_conversation` (explicit cascade), `add_message` (role-validated), `save_run` (result_dict serialised to JSON). `frontend2/server.py` gains 5 new REST routes (`GET/POST /api/conversations`, `GET/PATCH/DELETE /api/conversations/<id>`) and a persistence extension to `POST /api/run`: auto-creates a conversation from the query if no `conversation_id` supplied, saves the run, injects `_conversation_id` + `_run_id` into the SSE `result` event — all wrapped in a best-effort `try/except` so a store failure CANNOT break the SSE stream. The `demo`/`replay`/`live`/`simulate` run paths all persist. `result_json` stores run results locally (single-user SQLite, never leaves the machine — data boundary preserved). Review fixes applied: PATCH on a missing ID now returns 404 (was silent 200), `datetime.utcnow()` → `datetime.now(timezone.utc)`, temp-dir teardown leak plugged.
- Gates: **794 tests GREEN** (17 store unit + 34 frontend2 = 51 new; no regressions; Gate 1 run by pre-push hook on push) · Gate-2 independent review **Approved** (no Critical/Important findings; 4 Minor polish items — 3 applied as review-fix commit, 1 deferred) · Gate-5 functional verification **VERIFIED** (all 11 demo steps, 7 API endpoint checks, run-persistence + `_conversation_id` injection confirmed, store-failure-safety confirmed, adversarial boundary checks confirmed, stdlib-only confirmed) · secrets clean (`RohanOnly/` gitignored; no external calls; no tokens) · stdlib runtime intact (store.py = sqlite3/json/uuid/pathlib/os/datetime only).
- Gaps/Follow-ups: (1) `test_run_with_existing_conversation` does not assert `_conversation_id` in the result equals the pre-existing `cid` — latent gap noted by verifier, non-blocking. (2) `dev/run-tests.sh` already included the `frontend2` suite on `origin/main`; no change needed. (3) The `moat.sqlite` symlink is needed in the worktree for the full suite to run (same as every worktree-based gate run); documented. (4) Wave-2 (React front-end UI wiring to the persistence API) is deferred.
