...
- Built-By: `rohan` (reviewed/gated/merged by Head Claude). Reproduces Loka's EXACT rescue-ranking (per the user's "use what Loka used"). v1 used a K+1 SENTINEL for cross-heap rows (an approximation biasing union order — put MTOR at a sentinel-inflated rank). **v2 computes TRUE global within-partition ranks**: `_build/build_moat_db.py` accumulates each `(query, effect, ref_type)` partition fully and ranks ALL rows — `rank_cosine`=(cosine,euclidean,ref) asc, `rank_euclidean`=(euclidean,cosine,ref) asc, union survival `rank_cosine<=K OR rank_euclidean<=K`, `union_rank = rank_cosine + rank_euclidean`, no sentinel. `MoatClient.neighbors()` orders by `union_rank ASC`, top-K per ref_type; degrades to `[]` on an old-schema DB. `moat/facts.py` adds a `rescue_genes` category + annotated `supporting_genes`. **Schema**: `rank` → `rank_cosine/rank_euclidean/union_rank` (11-col); canonical `RohanOnly/moat/moat.sqlite` rebuilt from the 38.4M-row parquet at merge.
- Gates: **Gate 1 GREEN** · independent Gate-2 review (2 Important v1 fidelity fixes applied: TRUE global ranks + tested-helper/production tiebreak parity) · **Gate 5**: rebuilt from real parquet; TSC2 gene-rescue top by union_rank (DCTN6, FZD7, RALA…); **MTOR at its TRUE rank (union_rank 93)** · data boundary intact · stdlib (pyarrow only in `_build/`) · secrets clean. **Accepted data reality:** EP-distance rescue ≠ mechanistic for COMPOUNDS (rapamycin not in TSC2 EP top-K, verified); gene-rescue is the meaningful case. See [[wo5-moat-rescue-data-reality]].
- Gaps/Follow-ups: `supporting_genes>1` aggregation deferred; a future literature/pathway-annotation layer for compound rescue parked.
