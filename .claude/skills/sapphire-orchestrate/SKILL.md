...
**For MULTIPLE genes, do ONE batch call, NOT per-gene — and default to CAPTURED (no `--live`):** `python sapphire-orchestrator/orchestrator_tools.py emet --gene <PERTURBATION> --batch G1,G2,…,Gn`. This returns the cached BLIND EMET dossier **INSTANTLY**, and that dossier IS real captured live-EMET evidence (cited, FOR-vs-AGAINST, blind). This is the reliable default — use it. **Do NOT add `--live` unless the user explicitly asks for a fresh pull AND the Chrome worker is confirmed looping** — a half-alive worker that grabs the task but stalls makes `--live` hang for minutes. **WAIT for EMET to fully return before you launch ANY semantic agents** (they need the EMET evidence). (Single gene: `emet --gene <GENE>`.)
...
Use the **Quiver moat as a strong CORROBORATING signal** (not the primary ranker): the moat rank + `cosine_distance` (smaller = stronger) ADD weight — a gene strong in BOTH EMET and the moat is your highest-confidence call. Surface where they agree, and flag a moat↔literature contradiction as a **DIVERGENCE** (strong moat, no published mechanism = Quiver alpha worth a wet-lab probe — call it out, but in an EMET-focused ranking it sits below the literature-backed hits until evidence appears).
**ESM — you MUST address it, weighted appropriately (never silently drop it):** record each gene's `esm_signal` (its embedding proximity to the target) AND reference ESM in the synthesis. Weight it correctly: ESM-2 measures sequence/structure relatedness, NOT rescue — so high proximity is only weak positive context, and it earns weight ONLY when it coincides with a mechanism/EMET signal. **Sanity-check it against your ranking:** if the ESM-nearest genes are NOT your top rescue calls (e.g. here the ESM-top SSU72/RPS3/SAP18 are no-effect), state explicitly that "ESM-to-target proximity does not track rescue in this set, so it is down-weighted" — that honest calibration IS using it properly. The **semantic-agent verdicts** are further enrichment (supporting, never the sole basis).
  ...
  "ranked_genes": [
    {
      ...
      "emet_support": "strong | moderate | weak | none",
      "verdict": "rescue | partial | no-effect | worsen | divergence | not-a-moat-prediction",
