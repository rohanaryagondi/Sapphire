% SCS_MovieSplit_v16.m
% =====================================================================
% Pipeline v16 splitter (multi-protocol-step aware).
%
...
%
% Per-FOV outputs (under PLATE_DIR\v16_traces\):
...
% ── Configuration ───────────────────────────────────────────────────────
PLATE_DIR  = 'R:\QNP\2026_QNP\2026-05-20_JenniferGrooms_FireflyOne\PlateNumber_4';
OUTPUT_DIR = fullfile(PLATE_DIR, 'v16_traces');
...
%     % ── QC: stim raster per source ────────────────────────────────────────
%     if stim_step.found && ~isempty(fieldnames(stim_frames_per_source))
%         src_fields = fieldnames(stim_frames_per_source);
%         n_plot     = numel(src_fields);
% 
%         % collect (source_idx, frame) pairs
%         all_frames = [];
%         all_rows   = [];
%         for si = 1:n_plot
%             fld   = src_fields{si};   % renamed from 'f' to 'fld'
%             p1    = stim_frames_per_source.(fld).part1(:);
%             p2    = stim_frames_per_source.(fld).part2(:) + stim_split_local;
%             frs   = [p1; p2];
%             if isempty(frs), continue; end
%             all_frames = [all_frames; frs];
%             all_rows   = [all_rows;  repmat(si, numel(frs), 1)];
%         end
% 
%         figure('Name', sprintf('Stim raster — %s', fov_name), ...
%                'Position', [100 100 1200 max(300, n_plot*8)]);
%         if ~isempty(all_frames)
%             scatter(all_frames, all_rows, 2, 'b', 'filled');
%         else
%             text(0.5, 0.5, 'No stim frames found', ...
%                  'Units', 'normalized', 'HorizontalAlignment', 'center');
%         end
%         xlabel('Frame');
%         ylabel('Source index');
%         title(sprintf('%s — stim raster (%d sources)', fov_name, n_plot));
%         yline(0.5:1:n_plot+0.5, 'Color', [0.8 0.8 0.8], 'LineWidth', 0.3);
%         ylim([0.5, n_plot + 0.5]);
%         xlim([0, stim_n]);
%         set(gca, 'YTick', 1:n_plot, 'YTickLabel', src_fields, 'FontSize', 6);
%         drawnow;
%     end
