...
def write_yaml(seq: str, smi: str, path: Path) -> None:
    """Boltz YAML for one protein + one ligand + affinity readout.
    ...
    try:
        ...
        # Plain string fallback; SMILES are still wrapped in single quotes.
        path.write_text(
            "version: 1\n"
            "sequences:\n"
            "  - protein:\n      id: A\n      sequence: %s\n"
...
def main() -> None:
    ...
    for i, c in enumerate(complexes):
        ...
        yml = odir / "in.yaml"
        write_yaml(c["protein_seq"], c["smiles"], yml)
