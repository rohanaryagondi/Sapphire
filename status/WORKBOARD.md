...
| `semantic-corpora` (6) | **2/6 merged (#76, #83); +1 in review (clinical-trial-registry)** | Dual-source corpora for **~~patent-ip ✅~~ · post-market-safety · ~~clinical-trial-registry (PR open)~~ · payer-market-access · manufacturing-cmc · ~~dea-scheduling ✅~~**, per the **locked** method. **patent-ip** (#76) + **dea-scheduling** (#83) merged. **clinical-trial-registry shipped (PR open):** 12 verified cards, **all T1 `clinicaltrials.gov`** (CT.gov v2 API) — CNS termination-signals (AD anti-amyloid futility, ALS incl. the BIIB105 ATXN2 ASO, HD HTT-ASOs, the ENDEAR positive-stop lesson); suite 571. **EMET host-permission now GRANTED** (verified — the chat UI reads) → Pass-B backfill for the 3 shipped + the EMET-*central* **post-market-safety** are now buildable (dedicated EMET pass = next). manufacturing-cmc held (fda.gov 403; 5 T2 cards researched); payer-market-access (cms.gov 403). | — | [tools](tools.md) · [**brief**](../docs/superpowers/plans/2026-06-23-semantic-corpora-delegation.md) |
