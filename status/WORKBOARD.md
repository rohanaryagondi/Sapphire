...
| `semantic-corpora` (6) | **1/6 in review (patent-ip, PR open)** | Dual-source corpora for **~~patent-ip~~ · post-market-safety · clinical-trial-registry · payer-market-access · manufacturing-cmc · dea-scheduling**, per the **locked** method. **patent-ip shipped Pass-A-first**: 7 verified granted-patent cards (ASO/SM/AAV/platform, SMA-franchise-anchored), `validate-corpus.sh` checks CLEAN, suite 478. Pass B (EMET) pending the Claude-extension host-permission for `emet.benchsci.com`; T1 re-tier pending HELP `patent-ip-t1-patent-domains`. Then batch the remaining 5. | `hayes/corpus-patent-ip` (PR open) | [tools](tools.md) · [**brief**](../docs/superpowers/plans/2026-06-23-semantic-corpora-delegation.md) |
