...
| `semantic-corpora` (6) | **3/6 merged (#76, #83, #85)** | Dual-source corpora for **~~patent-ip ✅~~ · post-market-safety · ~~clinical-trial-registry ✅~~ · payer-market-access · manufacturing-cmc · ~~dea-scheduling ✅~~**, per the **locked** method. **patent-ip** (#76) + **dea-scheduling** (#83) + **clinical-trial-registry** (#85, squash `c0f100d`) merged. **clinical-trial-registry:** 12 verified cards, **all T1 `clinicaltrials.gov`** (CT.gov v2 API) — CNS termination-signals (AD anti-amyloid futility, ALS incl. the BIIB105 ATXN2 ASO, HD HTT-ASOs, the ENDEAR positive-stop lesson); gate CLEAN, suite 571, anti-fabrication spot-verified (4/4 cards field-for-field vs live API). **EMET host-permission now GRANTED** (verified — the chat UI reads) → Pass-B backfill for the 3 shipped + the EMET-*central* **post-market-safety** are now buildable (dedicated EMET pass = next). manufacturing-cmc held (fda.gov 403; 5 T2 cards researched); payer-market-access (cms.gov 403). | — | [tools](tools.md) · [**brief**](../docs/superpowers/plans/2026-06-23-semantic-corpora-delegation.md) |
