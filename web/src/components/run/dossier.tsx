...
import { FlagChip, PlaneChip, ProvChip, TierChip } from "@/components/ui/chips";
...
function FactCard({
  ...
  return (
    <button
      ...
      className={cn(
        "card-hover group w-full rounded-[var(--radius)] border border-[var(--color-border)] bg-[var(--color-panel)] p-2.5 text-left",
        active && "border-[var(--color-border-focus)] bg-[var(--color-panel-raised)]",
      )}
    >
      <div className="flex items-start gap-2">
        <p className="flex-1 text-[13px] leading-snug text-[var(--color-fg)]">
          {fact.value}
        </p>
